...
def ldm_conditional_sample_one_mask(
    ...
    with torch.no_grad(), torch.amp.autocast("cuda"):
        ...
        # synthesize latents
        if (
            isinstance(noise_scheduler, DDPMScheduler)
            and num_inference_steps < noise_scheduler.num_train_timesteps
        ):
            warnings.warn(
                "**************************************************************\n"
...
def ldm_conditional_sample_one_image(
    ...
    with torch.no_grad(), torch.amp.autocast("cuda"):
        ...
        end_time = time.time()
        logging.info(
            f"---- DM/ControlNet Latent features generation time: {end_time - start_time} seconds ----"
        )
        del (
            unet_inputs,
            controlnet_inputs,
...
class LDMSampler:
    ...
    def __init__(
        ...
        if len(self.controllable_anatomy_size):
            ...
            self.anatomy_list = [
                label_dict[organ_and_size[0]] for organ_and_size in self.controllable_anatomy_size
            ]
    ...
    def sample_multiple_images(self, num_img):
        ...
        for index_s in range(len(selected_mask_files)):
            ...
            if len(self.controllable_anatomy_size) > 0:
                ...
                # mask augmentation
                if if_aug:
                    combine_label_or = augmentation(
                        combine_label_or, self.output_size, self.random_seed
                    )
            end_time = time.time()
            ...
            if pass_quality_check or (num_img - num_generated_img) >= (
                len(selected_mask_files) - index_s
            ):
    ...
    def prepare_one_mask_and_meta_info(self, anatomy_size_condtion):
        ...
        top_region_index_tensor = (
            torch.FloatTensor(top_region_index).unsqueeze(0).half().to(self.device) * 1e2
        )
        bottom_region_index_tensor = (
            torch.FloatTensor(bottom_region_index).unsqueeze(0).half().to(self.device) * 1e2
        )
    ...
    def find_closest_masks(self, num_img):
        ...
        for c in candidates:
            ...
            for axis in range(3):
                ...
                diff += abs(
                    (
                        abs(c["dim"][axis] * c["spacing"][axis])
                        - self.output_size[axis] * self.spacing[axis]
                    )
                    / 10
                )
                # check diff in dim
