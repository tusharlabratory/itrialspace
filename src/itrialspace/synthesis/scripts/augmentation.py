    ...
    if tumor_szie > 0:
        ...
        while True:
            ...
            distored_mask = elastic(
                (real_l_volume > 0).cuda(), spatial_size=tuple(output_size)
            ).as_tensor()
...
def augmentation_tumor_liver(pt_nda, output_size, random_seed=None):
    ...
    while True:
        ...
        real_l_volume = elastic(
            (real_l_volume == 2).cuda(), spatial_size=tuple(output_size)
        ).as_tensor()
        # get organ mask
    ...
    if tumor_szie > 0:
        ...
        while True:
            ...
            if cnt < 20:
                # random distor mask
                distored_mask = elastic(
                    (real_l_volume == 1).cuda(), spatial_size=tuple(output_size)
                ).as_tensor()
                real_l_volume = distored_mask * organ_mask
            elif 20 <= cnt < 40:
                threshold = 0.75
