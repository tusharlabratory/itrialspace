...
class PNDMScheduler(Scheduler):
    ...
    def step_prk(
        ...
        diff_to_prev = (
            0 if self.counter % 2 else self.num_train_timesteps // self.num_inference_steps // 2
        )
        ...
        prev_sample: torch.Tensor = self._get_prev_sample(
    ...
    def step_plms(self, model_output: torch.Tensor, timestep: int, sample: torch.Tensor) -> Any:
        ...
        if len(self.ets) == 1 and self.counter == 0:
            ...
            model_output = (1 / 24) * (
                55 * self.ets[-1] - 59 * self.ets[-2] + 37 * self.ets[-3] - 9 * self.ets[-4]
            )
