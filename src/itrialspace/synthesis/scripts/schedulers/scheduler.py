...
def _scaled_linear_beta(num_train_timesteps: int, beta_start: float = 1e-4, beta_end: float = 2e-2):
    ...
    """
    return (
        torch.linspace(beta_start**0.5, beta_end**0.5, num_train_timesteps, dtype=torch.float32)
        ** 2
    )
...
def _sigmoid_beta(
    num_train_timesteps: int, beta_start: float = 1e-4, beta_end: float = 2e-2, sig_range: float = 6
):
    """
...
class Scheduler(nn.Module):
    ...
    def __init__(
        self, num_train_timesteps: int = 1000, schedule: str = "linear_beta", **schedule_args
    ) -> None:
        super().__init__()
    ...
    def add_noise(
        ...
        sqrt_alpha_cumprod: torch.Tensor = unsqueeze_right(
    ...
    def get_velocity(
        ...
        sqrt_alpha_prod: torch.Tensor = unsqueeze_right(
