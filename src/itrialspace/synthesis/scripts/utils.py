...
def get_index_arr(img):
    ...
    return np.moveaxis(
        np.moveaxis(
            np.stack(
                np.meshgrid(
                    np.arange(img.shape[0]), np.arange(img.shape[1]), np.arange(img.shape[2])
                )
            ),
            0,
            3,
    ...
    for label in target_label:
        ...
        if len(dominant_vals) >= 2:  # Case: no predictions
            new_background = np.logical_or(
                new_background,
                np.logical_not(
                    np.logical_or(label_cc == dominant_vals[0], label_cc == dominant_vals[1])
                ),
            )
...
def prepare_maisi_controlnet_json_dataloader(
    ...
    common_transform = [
        ...
        Lambdad(
            keys="top_region_index", func=lambda x: torch.FloatTensor(x), allow_missing_keys=True
        ),
        Lambdad(
            keys="bottom_region_index", func=lambda x: torch.FloatTensor(x), allow_missing_keys=True
        ),
...
def prepare_maisi_controlnet_infer_dataloader(
    ...
    infer_ds = CacheDataset(
        data=list_infer, transform=infer_transforms, cache_rate=cache_rate, num_workers=8
    )
    infer_loader = DataLoader(
        infer_ds, batch_size=batch_size, shuffle=False, num_workers=2, pin_memory=False
    )
    return infer_loader
...
def general_mask_generation_post_process(volume_t, target_tumor_label=None, device="cuda:0"):
    ...
    body_region_mask = (
        erode_one_img(torch.from_numpy((volume_t > 0)).to(device), filter_size=3, pad_value=0.0)
        .cpu()
        .numpy()
    )
    ...
    if target_tumor_label == 23 and np.sum(target_tumor) > 0:
        ...
        dia_lung_tumor_mask = (
            dilate_one_img(torch.from_numpy((data == 23)).to(device), filter_size=3, pad_value=0.0)
            .cpu()
            .numpy()
        )
        tmp = (
            (data * (dia_lung_tumor_mask.astype(np.uint8) - (data == 23).astype(np.uint8)))
            .astype(np.float32)
            .flatten()
        )
        tmp[tmp == 0] = float("nan")
        ...
        if mode in [28, 29, 30, 31, 32]:
            dia_lung_tumor_mask = (
                dilate_one_img(
                    torch.from_numpy(dia_lung_tumor_mask).to(device), filter_size=3, pad_value=0.0
                )
                .cpu()
                .numpy()
            )
            lung_remove_mask = dia_lung_tumor_mask.astype(np.uint8) - (data == 23).astype(
                np.uint8
            ).astype(np.uint8)
            data[
    ...
    if target_tumor_label == 26 and np.sum(target_tumor) > 0:
        ...
        dia_tumor_mask = (
            dilate_one_img(
                torch.from_numpy(dia_tumor_mask).to(device), filter_size=3, pad_value=0.0
            )
            .cpu()
            .numpy()
        )
...
class MapLabelValue:
    ...
    def __init__(
        self, orig_labels: Sequence, target_labels: Sequence, dtype: DtypeLike = np.float32
    ) -> None:
        """
        Args:
...
def dynamic_infer(inferer, model, images):
    ...
    if torch.numel(images[0:1, 0:1, ...]) <= math.prod(inferer.roi_size):
        ...
        if len(orig_roi) != len(spatial_dims):
            raise ValueError(
                f"ROI length ({len(orig_roi)}) does not match spatial dimensions ({len(spatial_dims)})."
            )
